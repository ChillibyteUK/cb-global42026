 ...
 * img/icons/ (where cb_get_icon_choices()/cb_icon() in inc/utilities.php
 ...
 * templating. Reuses cb_get_icon_choices()/cb_icon() from inc/utilities.php
 * (loaded after this file, but that's fine — this only runs later, from
 * the acf/render_field hook below, by which point every inc/ file has
 * already been required) instead of re-globbing img/icons/ itself.
 *
 * SVG markup is echoed as-is, same trust model as cb_icon() on the
 * frontend — anything in img/icons/ already went through
 * cb_sanitise_svg_markup() on the way in via this same file, or was placed
 * there directly by a developer, so it isn't re-sanitised here.
 *
 * @return string HTML grid, or a plain "no icons yet" message.
 */
function cb_render_icon_grid() {
	$choices = cb_get_icon_choices();
	...
	foreach ( array_keys( $choices ) as $slug ) {
		$items .= '<div style="width:100px;text-align:center;">'
			...
			. cb_icon( $slug ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- already-sanitised theme-owned SVG, same trust model as cb_icon()'s frontend usage.
			. '</div>'
...
add_action(
	...
	function () {
		echo cb_render_icon_grid(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- deliberately bypassing acf_esc_html()'s wp_kses('acf'), see comment above; content is our own already-sanitised SVGs plus hardcoded layout markup, not user input.
	}
);
 ...
 * @return array
 */
function cb_allow_icon_svg_upload( $mimes ) {
...
add_filter( 'upload_mimes', 'cb_allow_icon_svg_upload' );
 ...
 * @return array
 */
function cb_allow_icon_svg_filetype( $data, $file, $filename, $mimes ) {
...
add_filter( 'wp_check_filetype_and_ext', 'cb_allow_icon_svg_filetype', 10, 4 );
 ...
 * not explicitly listed. This is sanitised once at upload time before it's
 * written to disk, not re-processed on every frontend render.
 *
 * @param string $svg_markup Raw SVG markup.
 * @return string Sanitised SVG markup.
 */
function cb_sanitise_svg_markup( $svg_markup ) {
	...
	// Single-colour icons with no fill anywhere in the source don't
	// otherwise respect the surrounding text colour — SVG's fill defaults
	// to black regardless of CSS context. Injecting fill="currentColor" on
	// the root <svg> makes them behave like stroke="currentColor" icons
	// already do. Only when nothing already specifies a fill: a genuinely
	// multi-colour icon that hardcodes different fills per path is left
	// alone rather than flattened to one colour.
 ...
 * @return void
 */
function cb_handle_icon_upload( $post_id ) {
	if ( 'options' !== $post_id ) {
		return;
	}
	...
	$svg_markup = cb_sanitise_svg_markup( $svg_markup );

	// Icon slug comes from the original uploaded filename, not a separate
	// "name this icon" field — matches cb_get_icon_choices()'s own
	// convention of the filename being the icon's identity.
